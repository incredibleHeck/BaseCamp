...
7. **Phase 3 (enterprise demo):** Screening alerts use the `senAlerts` collection. Create matching Auth users if you use the administrative login forms (email + password). Demo defaults include `schooladmin@basecamp.com`, `sen_coordinator@basecamp.com`, `circuit_supervisor@basecamp.com`, and `superadmin@basecamp.com` (or `super_admin@basecamp.com`) for super admin — use the same demo password (e.g. `HecTech@2026!`) and set `role` on Firestore `users/{uid}` (`teacher`, `headteacher`, `org_admin`, `school_admin`, `sen_coordinator`, `circuit_supervisor`, `super_admin`). Prefer **`organizationId`** for B2B scope (multi-branch networks); legacy docs may still use `districtId` — the app merges these on read. Also set optional `circuitId` / `schoolId` where applicable.
