...
7. **Phase 3 (enterprise demo):** Screening alerts use the `senAlerts` collection. Create matching Auth users if you use the new login tiles (e.g. `headteacher@basecamp.com`, `district@basecamp.com`, `sen_coordinator@basecamp.com`, `circuit_supervisor@basecamp.com`, `super_admin@basecamp.com`) with the same password as other demo accounts, and set `role` (`teacher`, `headteacher`, `district`, `sen_coordinator`, `circuit_supervisor`, `super_admin`) and optional `districtId` / `circuitId` / `schoolId` on their Firestore `users/{uid}` docs.
